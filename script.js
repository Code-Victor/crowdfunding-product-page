...
function disableScrolling() {
    setTimeout(function(){
        pledgeModal[0].getElementsByClassName('modal')[0].classList.add("change-scrollbar");
    },1500)
...
function close_input() {
    for (let index = 0; index < pledges.length; index++) {
        ...
        if ('none' in element.classList){}
            ...
            element.parentNode.parentNode.querySelector("input[type='radio']").removeAttribute('checked');
        }
...
function enableScrolling() {
    pledgeModal[0].getElementsByClassName('modal')[0].classList.remove("change-scrollbar");
}
